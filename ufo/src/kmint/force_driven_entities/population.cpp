...
void Population::getParents() {
    ...
    for (int i = 0; i < 100; i++) {
        ...
        std::cout << " Fitness of Human " << i << " is " << humanproperties[i].Fitness << std::endl;
    }
    std::cout << std::endl;
...
void Population::SetRandomMutation(int i){
    ...
    switch (GenToMutate) { 
        case 0:
            ...
            break;
        case 1:
            std::cout << " Separation mutated from " << humans[i]->SeparationWeight
                      << " to ";
        ...
        case 2:
            ...
            break;
        case 3:
            std::cout << " RedTank mutated from "
                      << humans[i]->RedTankWeight << " to ";
            humans[i]->RedTankWeight    = RandomNumber(-1, 1);
            std::cout << humans[i]->RedTankWeight << std::endl;

            break;
        case 4:
            std::cout << " GreenTank mutated from " << humans[i]->GreenTankWeight
                      << " to ";
            ...
            break;
        case 5:
            std::cout << " Ufo mutated from "
                      << humans[i]->UfoWeight << " to ";
            humans[i]->UfoWeight        = RandomNumber(-1, 1);
            std::cout << humans[i]->UfoWeight << std::endl;

            break;
        case 6:
            std::cout << " Door mutated from " << humans[i]->UfoWeight << " to ";
